/*
    ...
    WARING:
        IF YOU UPDATE THE VISITOR API VERSION, YOU WILL NEED TO UPDATE IT IN CI.ASOSSERVICES.COM/CORE/CORE[.*].JS
        THAT FILE IS MANAGED BY THE CUSTOMER INTELLIGENCE TEAM (MVT)
...
/* END VisitorAPI 1.3.2 */


/* START VisitorAPI_v1.js */

if(typeof visitor === "undefined"){
   var visitor = new Visitor("C0137F6A52DEAFCC0A490D4C@AdobeOrg");
}

var host=window.location.hostname
if (host != 'undefined' && host != null) {
    if (host.indexOf(".fr") > -1) {
        visitor.trackingServer = "metric.asos.fr";
        visitor.trackingServerSecure = "metrics.asos.fr";
        visitor.marketingCloudServer = "metric.asos.fr";
        visitor.marketingCloudServerSecure = "metrics.asos.fr";
    } else if (host.indexOf(".de") > -1) {
        visitor.trackingServer = "metric.asos.de";
        visitor.trackingServerSecure = "metrics.asos.de";
        visitor.marketingCloudServer = "metric.asos.de";
        visitor.marketingCloudServerSecure = "metrics.asos.de";
    } else {
        visitor.trackingServer = "metric.asos.com";
        visitor.trackingServerSecure = "metrics.asos.com";
        visitor.marketingCloudServer = "metric.asos.com";
        visitor.marketingCloudServerSecure = "metrics.asos.com";
    }
}

/* START AppMeasurement_v1.js */
...
s.account = "asoscomprod";
s.visitorNamespace = "asos";
s.visitor = Visitor.getInstance("C0137F6A52DEAFCC0A490D4C@AdobeOrg");

CalculateCustomTime();

function CalculateCustomTime() {
    var d = new Date();
    var currentTimeZone = -d.getTimezoneOffset() / 60;
    var a = String(currentTimeZone).split('.');
    if (a.length >= 1) {
        if (a[0] < 10) {
            if (a[0] < 0) {
                currentTimeZone = '-0' + String(-a[0]);
            } else {
                currentTimeZone = '0' + String(a[0]);
            }
        }
        if (a.length == 2) {
            currentTimeZone = currentTimeZone + (parseInt(a[1]) * 60 / 10);
        } else {
    ...
    if (CurrentHrs < 10) {
        CurrentHrs = '0' + String(CurrentHrs);
    }
    ...
    var output = CurrentDate + '/' + CurrentMonth + '/' + d.getFullYear() + ' ' + CurrentHrs + ':' + CurrentMin + tempstr + currentTimeZone;
    s.prop5 = output;
}

/************************** ADOBE TARGET - RELATED VARIABLES (THEO 18/11/14) *********************/
s.eVar9 = typeof mvtTestName !== "undefined" ? mvtTestName : "";
s.list1 = (typeof mvtTargetNames !== "undefined" && Object.prototype.toString.call(mvtTargetNames) === '[object Array]') ? mvtTargetNames.join(",") : [].join(",");
...
s.linkInternalFilters = "javascript:,asos.com,asos.fr,asos.de,asos.cn,asosservices.com,backoffice.com"
s.linkLeaveQueryString = false
s.linkTrackVars = "prop1,prop2,prop3,eVar6,prop5,prop7,prop8,eVar2,eVar9,eVar10,eVar31,hier1,server,channel,campaign"
...
function s_doPlugins(s) {
    /* OPTIONAL Visitor ID Present? */
    s.prop29 = (typeof (Visitor) != "undefined" ? "VisitorAPI Present" : "VisitorAPI Missing");

    /* TEMP BACKUP FOR CASE SENSITIVE - External Campaign Tracking */
    //s.eVar12 = s.Util.getQueryParam('affId');
    //if (s.Util.getQueryParam('affId')) {
    //    s.events = s.events + ",event6"
    //}

    /* External Campaign Stacking */
    s.eVar35 = s.crossVisitParticipation(s.eVar12, 's_ev35', '30', '5', '>', '', 1);

    /* Internal Search Stacking */
    if (s.events.indexOf("event3") >= 0 || s.events.indexOf("event4") >= 0) {
        s.eVar3 = s.crossVisitParticipation(s.eVar25, 's_ev3', '30', '5', '>', '', 1);
    }
    ...
    //s.contextData['page_url'] = window.location.href;
    s.prop10 = s.getPreviousValue(s.pageName, 'gpv_p10', '');

    /* Set Custom Product View Event */
    if (s.events.indexOf("prodView") >= 0) {
        s.events = s.events + ",event1"
    }
    s.prop34 = s.getPercentPageViewed(s.prop10); //get array of data on prev page % viewed
    /* Set custom time stamp */
    CalculateCustomTime();
    
    /* performance metrics - Rob Brown */
    if(typeof(window.performance) !== 'undefined'){
        var navApi = window.performance.timing;
        s.eVar29 = navApi.responseStart - navApi.navigationStart; //first byte
        s.eVar30 = navApi.domContentLoadedEventEnd - navApi.navigationStart; //dom ready
    }
}
s.doPlugins = s_doPlugins
/************************** PLUGINS SECTION *************************/
/*Plugin for s.prop34 Start*/
//Plugin Code
...
/* LAST UPDATED: 06-05-2013 APP MEASUREMENT JS COMPATIBLE */
if(!s.__ccucr){
    s.c_rr = s.c_r;
    s.__ccucr = true;
    function c_r(k){
...
if (host != 'undefined' && host != null) {
    if (host.indexOf(".fr") > -1) {
        s.trackingServer = "metric.asos.fr"
        s.trackingServerSecure = "metrics.asos.fr"
    } else if (host.indexOf(".de") > -1) {
        s.trackingServer = "metric.asos.de"
        s.trackingServerSecure = "metrics.asos.de"
    } else {
        s.trackingServer = "metric.asos.com"
        s.trackingServerSecure = "metrics.asos.com"
    }
}
