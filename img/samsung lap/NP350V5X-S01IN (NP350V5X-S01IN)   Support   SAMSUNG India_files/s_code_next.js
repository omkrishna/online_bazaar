...
function getOmniTopSection(){
	...
	if(splitURL[2].indexOf("business")!=-1){
		return "business";
	}else if(splitURL[2].indexOf("support")!=-1){
		return "support";
	}else{
		return "";
	}
...
s.linkInternalFilters="javascript:,tel:,www.samsung.com,japan.samsung.com,2.samsung.com,3.samsung.com,4.samsung.com,shop.samsung.com,store.samsung.com";
...
function sendPageCode(group, category){
	...
	try{
		...
		if(pageType != ""){
			...
			}else if(pageType.indexOf("product category") != -1 || pageType.indexOf("support category")!=-1 || (pageType.indexOf("business")!=-1 && pageType.indexOf("landing")!=-1) && pageType.indexOf("business support landing")==-1){
				var groupVal = [];
...
function sendScPurchase(order,pId,dOption,pMethod){
	...
	s.purchaseID=pId;
	s.eVar50=pId;
	s.linkTrackVars="eVar37,events,products,purchaseID";
...
function sendScPurchaseSucc(order,pId,dOption,pMethod,pOption){
	...
	s.purchaseID=pId;
	s.eVar50=pId;
	s.linkTrackVars="eVar37,events,products,purchaseID,prop38";
	s.linkTrackEvents="purchase";
	s.events="purchase";
	setClickVar();
	s.tl(true,"o","scPurchase");
...
function sendScPurchaseFail(order,pId,tValue){
	...
	s.products=order.toUpperCase().replace(/;;/gi, ';');
	s.purchaseID=pId;
	s.linkTrackVars="eVar36,products,purchaseID";
	s.linkTrackEvents="";
...
function sendScCheckoutStep(step){
	s.linkTrackVars="eVar36";
	s.linkTrackEvents="";
	s.eVar36= step;
	setClickVar();
...
function sendScTeleSucc(order,pId,dOption,pMethod,pOption){
	...
	s.purchaseID=pId;
	s.eVar50=pId;
	s.linkTrackVars="eVar36,eVar37,products,purchaseID,prop38";
	s.linkTrackEvents="";
	setClickVar();
...
function sendClickCode( typeName, data, option ){
	...
	var trackVar = 	{ 
		...
		*/
		...
		//< Ko ga-young >
		"account" : "events=event31|eVar31=data#0|type=o|trackname=account event",
		...
		"content_click" : "events=event26|eVar26=data#0|products=data#1|type=o|trackname=content interaction",
		...
		//my samsung
		"mys_account": "events=event31|eVar31=data#0|products=data#1|type=o|trackname=my samsung account event",
		...
		if ( splitedTrackValue.indexOf("events") != -1 ){	// s.events
			...
			if ( trackValue[i].split("=")[1].indexOf("data#") != -1 ) {
				...
				if(typeName == "note4hub"){
					...
					s[eVar] = data.split(",")[number].replaceAll("'","");
				} else {
					s[eVar] = data.split(",")[number];
				}
				number = 0;
